...
probs_stochtree_warmstart = matrix(NA, cycles, M+2)
...
idx_mcmc = (num_gfr + num_burnin +1):num_samples
idx_dart = 1:((num_gfr + num_burnin)/2)
...
# 
# 
# stochtree_fit_warmstart <- stochtree::bart(
#   X_train = Xt, y_train = Yt,
#   num_trees = n_trees, num_gfr = num_gfr, num_burnin = num_burnin,
...
# idx_dart = 1:((num_gfr + num_burnin)/2)
# 
# variable_count_splits = stochtree_fit_warmstart$model_params$variable_count_splits[idx_mcmc,]
# alpha = stochtree_fit_warmstart$model_params$theta_used[-idx_dart]
# lpvs  = stochtree_fit_warmstart$model_params$lpvs[-idx_dart,]
# probs = colMeans(variable_count_splits >0)
# probs_stochtree_warmstart[i,] = probs
...
for(i in 1:cycles){
  ...
  # plot(probs_stochtree_warmstart[i,], main = paste0("Dart(Warmstart)",i), ylab = "PIP", ylim = c(0,1))
  plot(probs_stochtree[i,], main = paste0("Dart",i), ylab = "PIP", ylim = c(0,1))
}
